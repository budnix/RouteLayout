...
- **Heights**: every piece has a start height and a grade; set *height* to lift a whole connected group, *grade %* to build a ramp (pieces downstream rise with it). Ends only connect at matching heights — a helix shows its gap honestly. In 3D the track follows the profile with piers under viaducts; a track through a hill becomes a tunnel. **Levels**: a HUD button lists the heights found in the layout; pick one and everything else is greyed out and locked, so a two-level plan stays editable
