# AGENTS.md — knowledge base for AI coding agents working on RailSketch

RailSketch is a static web app for planning H0 model-railway layouts on PIKO A-Gleis track with a 3D preview. Live: https://budnix.github.io/RailSketch/ . This file explains how the code is organised, the conventions that are not obvious from reading it, and the traps previous agents fell into. Read it before changing anything.
...
| `js/catalog.js` | PIKO A-Gleis catalog with **generated geometry** and track **systems** (`SYSTEMS`, `toSystem`): 552xx = `piko-a`, generated 554xx mirrors = `piko-a-bed` (same `geo` object, `base` = 552xx id, unverified except 55418), turntable = `common`. Algorithms (fitter, closer) always produce base 552xx ids; the UI maps them to the selected system with `toCurrentSystem` at insertion time. Adding a brand = entries with a new `system` key (and a `SYSTEMS` entry); the palette lists whatever the catalog contains (`straight`, `curve`, `turnout`, `curvedTurnout`, `threeWay`, `wye`, `crossing`, `doubleSlip`), the turntable (`TT`, dynamic geometry), `geoOf(piece)`, `sampleSegment`. **PECO Setrack OO/HO** (`peco-setrack`, ids `ST-2xx`, 16.5 mm, 22.5° geometry, parallel 67 mm) is the first other brand; its values come from PECO's product descriptions (the site itself is egress-blocked), curved turnouts `verified: false`. `SYSTEMS[s]` carries `short`, `parallel`, `gauge` and `profile` (base system sharing geometry). `special: true` marks straights the decomposer must not use (transition/isolating tracks). |
| `js/layout.js` | The model: pieces, scenery, board, ports/connections, snapping, undo/redo, heights, turntable rim ports, JSON (de)serialisation, autosave. `ports()` pairs ports through a `SpatialHash` (cell 2.4 mm) and is cached together with `portsOf(piece)` (per-piece port arrays, O(1) `portOf`); `worldSegments(step)` is cached per step. Both caches are cleared in `emit()` and when a rim port is added, so any mutation must go through `emit`. |
| `js/templates.js` | Templates: exact multi-piece sets (`TEMPLATES`: passing siding left/right = WL/WR + G231 + G239 + G231 + WR/WL with two R9 counter-curves and a G239 on the siding; crossover left/right = turnout branch into the other turnout's branch, 61.88 mm parallel). Steps are `{ id, entry, from: [step, port] }`; `buildTemplate(key, startPose)` returns pieces without uids plus the main-line exit port; `editor.addTemplate(pieces, exit)` inserts them as one undo step and moves the cursor to that exit. The palette (PIKO tab) shows them in a "Templates" section with an outline icon; ids are mapped to the current track system. Tests assert every template closes (2 open ports for sidings, 4 for crossovers). Adding a template = one entry in `TEMPLATES` + a `tpl.<key>` string in the three languages. Templates are per base system (`TEMPLATES_BY_SYSTEM`, `templatesFor(system)`); Setrack has only crossovers because its siding does not close exactly. |
| `js/partlist.js` | `parsePartList(text)`: imports part lists from AnyRail, SCARM or a spreadsheet without assuming one format. Each line is scanned for a catalog article number (`55xxx`, any system) or a geometry code as a whole token (`G239`, `R2`, `WL`…); the quantity is a 1–999 integer that is not the article, a dimension (`239,07`, `30°`, `421 mm`) or a code, preferring one with a unit suffix (`x`, `szt`, `Stk`, `pcs`), else the first column when the line starts with a number, else the last. Header lines are skipped, duplicates summed, no quantity = 1. Lines with digits but no match are returned as `unknown`. The menu button writes the result into the `have` map (replacing per article). |
| `js/profile.js` | **System profile**: everything the algorithms know about a track palette, computed from the catalog per base system and cached: `straights` [[id,len]] (no flex/special), `radii` [{r, pieces:[[id,deg]]}], `turnouts` (standard L/R), `curvedTurnouts` [[left,right,rOut,rIn,deg]], `crossings` {deg:id} (double slip wins over a plain crossing), `headingUnit` (smallest angle ≥ 10°: PIKO 15, Setrack 11.25), `turnoutLen`, `unitStraight`, `minRealR`, `closePenalty`/`fitPenalty` (short pieces, small angles, unverified). `current.p` is the active profile; `useSystem(system)` sets it and every entry point (`fitStrokes`, `normalizeStroke`, `closeGap`) calls it with its `system` option (default PIKO), so nothing leaks between calls. normalize.js, fitter.js and closer.js read `P()`/`current.p` instead of hard-coded ids. Adding a brand = catalog entries with a new `system` + a `SYSTEMS` entry; the sketch fitter, closer, palette and part-list import follow automatically. |
| `js/spatial.js` | `SpatialHash(cell)`: uniform grid with integer cell keys; `add`/`near` for points (3×3 cells, so the query radius must be ≤ cell), `addBox`/`pairs()` for boxes (each candidate pair once). Correctness is checked against brute force in the tests; keep any new pair scan on it rather than writing a nested loop. |
| `js/editor2d.js` | Canvas 2D editor: pan/zoom/pinch, drag, select, cursor (active open end), sketch mode, rendering. |
| `js/fitter.js` | Sketch → pieces. `prepareStroke`, greedy fitter (`fitGreedy`), normalised pipeline (`fitNormalized`), `fitStrokes` (entry), `normalizeStroke` (live, on pointer-up), turnout placement, attachment to open ends. Branches off an **arc** get a curved turnout (`arcWithTurnouts`: BWL/BWR R2/R3 or R3/R4, toe scanned along the arc on the piece grid; the sketch's main line may be the outer route (exit 1) or the inner one (exit 2, via the `exit` field that `chain` honours). Two strokes whose interiors cross at ≈15° get a DKW (55224), at ≈30° a K30 (`findCrossings`/`polyCross`, ±7°); the crossing stroke is split at the intersection and both halves start from the crossing's other ports (`splitStroke`). A stroke recorded as crossing another is never matched as its branch. Mutual parenthood (both strokes start on each other) is resolved in favour of the longer stroke. |
...
| `js/checks.js` | Feasibility checks (`checkLayout`): grade > 3.5 %, clearance < 55 mm between crossing tracks on different levels, same-level collision (axes intersect, pieces not directly connected), axis spacing < 45 mm, track outside/within 20 mm of the board edge. Runs debounced after every change; results show as markers on the plan, a list in the menu and a badge on the menu button. Constants at the top of the file. Candidate pairs come from a `SpatialHash` over padded segment boxes (`GRID_CELL` 250 mm), so the cost is linear in the number of pieces; the closest-pair loop compares squared distances. Two pieces that both connect to the same neighbour (turnout legs) are not reported for spacing near that joint. Rolling-stock envelope: `STOCK` (short/standard/long: body length, pivot distance, `minRadius`), `envelopeHalf(R, stock, side)` = `HALF_WIDTH` 20 mm + the throw on the side facing the other track (`sideOf`: outer throw if the other track lies outside the arc, inner throw if inside, so concentric R1‖R2 needs 61.4 mm with standard stock and passes PIKO's 61.88), `requiredSpacing(rA, rB, stock, sideA, sideB)` adds `ENV_MARGIN`; pairs farther than `MIN_SPACING` but closer than that get type `envelope`, arcs tighter than `minRadius` get `radius`. `checkLayout(layout, { stock })`; the menu select stores the choice in the `stock` pref (default `standard`, which keeps PIKO's 61.88 mm geometry clean; `long` flags R1‖R2 and R1 itself). |
...
| `js/ui/app.js` | Shared UI context (`migrateStorage()` copies legacy `routelayout.*` keys to `railsketch.*` once, from the app's first name; keep it until 2027): `$`, `prefs` (`get/set/getJSON/setJSON` under `railsketch.*`), `toast`, `errMsg`, `createApp()` (Layout + Editor2D + View3D, global error toasts, `window.__railsketch = { layout, editor, view3d }`, `app.expose(obj)` merges into that hook). |
...
| `js/ui/levels.js` | Levels (layers) in 2D: HUD button with a count badge opens a bar with a select (all / one level) and up/down buttons. Levels come from `layout.levels()` (end heights clustered with a 30 mm gap, `LEVEL_GAP`), rebuilt on every change; nothing is declared by the user. Selecting a level calls `editor.setLevel({min,max,z})`: pieces whose height range (`Layout.zRange`) misses it are drawn as a faint grey trace, skipped by `hitTest`/open-port dots and dropped from the selection. Closing the bar or entering draw/train mode clears the filter. Not persisted (layout-specific). |
...
- Level filter: `editor.level` (`{min,max,z}` or null) with `editor.levelFilter()` for hit-testing and `Layout.inLevel(piece, level)`; `layout.hitTest(x, y, r, filter)` takes an optional predicate. New pieces are still inserted at the cursor's height, so inserting while filtered stays on the visible level.
...
- The palette shows a "Recently used" section (last 6 inserted ids/types, `railsketch.recent`) rebuilt after every insert while keeping the list scroll position.
...
- Errors are never silent: `window.error`, `unhandledrejection` and the custom `railsketch:error` event show a red toast (iPad has no console). `layout.emit` isolates listeners so a WebGL failure cannot abort an edit.
- Preferences live in `localStorage` under `railsketch.*` (`v1` = the layout itself, `lang`, `mode`, `grid`, `fix`, `side`, `tab`, `system` = selected track system, `have` = owned pieces per article for the shopping list (also filled by the part-list import), `recent`, `dims`, `stock` = rolling-stock profile for the checks). Layout-specific settings (board colour) go into the layout JSON instead.
...
- Insert pieces in browser tests through `window.__railsketch.insert(id, entry)` / `insert(type)`, not by poking DOM controls.
