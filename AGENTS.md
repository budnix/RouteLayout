...
| `js/templates.js` | Templates: exact multi-piece sets (`TEMPLATES`: passing siding left/right = WL/WR + G231 + G239 + G231 + WR/WL with two R9 counter-curves and a G239 on the siding; crossover left/right = turnout branch into the other turnout's branch, 61.88 mm parallel). Steps are `{ id, entry, from: [step, port] }`; `buildTemplate(key, startPose)` returns pieces without uids plus the main-line exit port; `editor.addTemplate(pieces, exit)` inserts them as one undo step and moves the cursor to that exit. The palette (PIKO tab) shows them in a "Templates" section with an outline icon; ids are mapped to the current track system. Tests assert every template closes (2 open ports for sidings, 4 for crossovers). Adding a template = one entry in `TEMPLATES` + a `tpl.<key>` string in the three languages. |
