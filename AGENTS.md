...
| `js/share.js` | Share links: the whole layout JSON deflate-raw-compressed (`CompressionStream`) and base64url-encoded into `#L=…` (`#J=…` uncompressed fallback for browsers without CompressionStream). `main.js` loads it on start and on `hashchange`, then clears the hash. |
